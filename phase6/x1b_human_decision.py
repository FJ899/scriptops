...
class AnchoredGitV2:
    ...
    def cas_main(self, new_oid: str, old_oid: str) -> bool:
        cp = self.run("update-ref", "refs/heads/main", new_oid, old_oid, check=False)
...
def execute_admission(
    ...
    base = admission["scriptops_base_head"]
    if git.main_head() != base:
        raise X1BError("FAILED_BASE_CHANGED before prospective construction")
    git.require_clean()
    commit, new_log, _ = _prospective_commit(git, local, admission, request)
    if git.main_head() != base:
        raise X1BError("FAILED_BASE_CHANGED before CAS")
...
def _postverify(
    ...
    if git.main_head() != commit or commit == base:
        raise X1BError("post-CAS main mismatch")
    _verify_prospective(git, base, commit, local, new_log)
...
def approve_scene(scene_id: str, decision_pr: int, root: Path = SOURCE_ROOT) -> dict[str, Any]:
    ...
    with x1b_lock(git):
        ...
        _scan_replay(base_log, admission["request_sha256"])
        if git.main_head() != local.base_head:
            raise X1BError("FAILED_BASE_CHANGED after authority read")
        git.require_clean()
